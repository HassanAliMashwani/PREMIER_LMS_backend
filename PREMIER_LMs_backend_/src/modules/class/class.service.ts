...
export class ClassService {
  ...
  async findUpcomingForStudent(userId: string) {
    ...
    const conditions = enrollments.map((e) => {
      if (e.batchName) {
        return {
  ...
  async findRecordingsForStudent(userId: string) {
    ...
    for (const enrollment of enrollments) {
      ...
      courseLectures.sort((a, b) => a.classNo - b.classNo);
      ...
      const mapped = courseLectures.map((lecture, index) => {
  ...
  async generateRecordingToken(lectureId: string, userId: string, userRole: string) {
    ...
    if (userRole !== 'admin' && userRole !== 'teacher') {
      ...
      if (!isBatchCompleted) {
        ...
        siblingLectures.sort((a, b) => a.classNo - b.classNo);

        const lectureIndex = siblingLectures.findIndex((l) => l.id === lecture.id);
        const multiplier = enrollment.course.lecturesPerLiveClass ?? 1;
        const unlockedCount = completedLiveCount * multiplier;
  ...
  async getDashboardHierarchy() {
    ...
    return batches.map((batch) => {
      ...
      const batchClasses = allDbClasses.filter(
        (cls) => cls.batchId === batch.id || (cls.batchName && cls.batchName.toLowerCase() === batch.name.toLowerCase())
      );
  ...
  async verifyRecordingToken(token: string, userId: string, userRole: string) {
    const lectureId = this.recordingTokenService.verifyAndBurn(token);
    if (!lectureId) {
    ...
    if (userRole !== 'admin' && userRole !== 'teacher') {
      ...
      if (enrollment) {
        ...
        const mappedLectures = siblingLectures.map((item, index) => {
          let isLocked = false;
          if (!isBatchCompleted) {
            const multiplier = enrollment.course.lecturesPerLiveClass ?? 1;
            const unlockedCount = completedLiveCount * multiplier;
    ...
    const mappedLectures = siblingLectures.map((item) => ({
      ...item,
      isLocked: false,
    }));
